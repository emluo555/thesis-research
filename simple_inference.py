...
def generate_inner_stage_beam(prompt, image_path, beam_size=2):
    ...
    input_text = processor.apply_chat_template(messages, add_generation_prompt=True)
    print("input_text, ", input_text, type(input_text), "prompt, ", prompt)
    inputs = processor(image, input_text, return_tensors='pt').to(device)
    ...
    for stage, end_marker in zip(stages, end_markers):
        ...
        for _ in range(beam_size):  
            ...
            inputs = processor(image, input_ids, return_tensors='pt').to(device)
            output = model.generate(**inputs, **generation_kwargs, output_attentions=True, return_dict_in_generate=True)

            # test extract attention
            attentions = output.attentions
            # print("output attns ", attentions)
